<?php
    ...
    abstract class BaseMailer extends \CComponent implements MailerInterface
    {
         ...
         */
        public $messageClass = 'zazymko\swiftmailer\BaseMessage';
        /**
        ...
        /**
         * @var \zazymko\swiftmailer\View|array view instance or its array configuration.
         */
        private $_view = [];
        ...
        public function setView($view)
        {
            if (!is_array($view) && !is_object($view)) {
                throw new \Exception('"' . get_class($this) . '::view" should be either object or configuration array, "' . gettype($view) . '" given.');
            }
            $this->_view = $view;
        }
        ...
        protected function createView(array $config)
        {
            if (!array_key_exists('class', $config)) {
                $config['class'] = 'zazymko\swiftmailer\View';
            }
        ...
        public function compose($view = null, array $params = [])
        {
            ...
            if (isset($text)) {
                ...
                $text = html_entity_decode(strip_tags($html), ENT_QUOTES | ENT_HTML5, Yii::app()->charset);
                // improve whitespace
        ...
        public function send($message)
        {
            ...
            Yii::log('Sending email "' . $message->getSubject() . '" to "' . $address . '"', \CLogger::LEVEL_INFO,
                __METHOD__);
        ...
        public function generateMessageFileName()
        {
            ...
            return date('Ymd-His-', $time) . sprintf('%04d',
                    (int)(($time - (int)$time) * 10000)) . '-' . sprintf('%04d', mt_rand(0, 10000)) . '.eml';
        }

        /**
         * @return string the directory that contains the view files for composing mail messages
         * Defaults to 'application.mail.views'.
         */
        public function getViewPath()
        {
            if ($this->_viewPath === null) {
                $this->setViewPath('application.mail');
            }
        ...
        {
            $event = new \CModelEvent($this, ['message' => $message]);
        ...
        {
            $this->raiseEvent(self::EVENT_BEFORE_SEND, $event);
        }
        ...
        {
            $event = new \CModelEvent($this, ['message' => $message, 'isSuccessful' => $isSuccessful]);
        ...
        {
            $this->raiseEvent(self::EVENT_AFTER_SEND, $event);
        }
    }